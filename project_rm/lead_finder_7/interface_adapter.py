"""Frontend interface contract adapter for lead_finder_7.
...
def get_frontend_contract() -> dict:
    ...
    return {
        ...
        "frontend_rule": "Build any GUI you want; do not duplicate backend validation or lead-scoring rules in the GUI.",
        ...
        "endpoints": [
            ...
            {"method": "POST", "path": "/leads", "auth_required": True, "purpose": "Find buyer/seller leads by product, region, optional city, and intent."},
            {"method": "GET", "path": "/history", "auth_required": True, "purpose": "List recent saved lead searches."},
        ],
        "frontend_must_send": ["JSON request bodies", "Content-Type: application/json", "Authorization header on protected routes"],
        "frontend_should_handle": ["400 validation errors", "401 login/session errors", "empty lead result states"],
    }
